...
Information.setConversionMode(Standard)


test "information arithmetics":


  # Assignable iteration: when the conversion gap is big, it doesn't take effect
  var foo = 1.tb
  for _ in 0..10000:
    foo = foo + 1.b
  
  #check foo.getValueAs(Gigabyte) == 1.0

  # Accumulative iteration: 
  var foo2 = 1.tb
  var accumulator = 0.b
  for _ in 0..100000:
    accumulator = accumulator + 1.b
  
  
  #check (foo2 + accumulator).getValueAs(Gigabyte) == 1.000093133188784